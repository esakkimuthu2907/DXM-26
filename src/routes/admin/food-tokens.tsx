...
export const Route = createFileRoute('/admin/food-tokens')({
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -40 }}
            className={`fixed top-6 right-6 z-[100] px-6 py-3 rounded-xl shadow-2xl font-semibold text-sm border ${toast.type === 'success' ? 'bg-green-900/80 border-green-500/50 text-green-300' : 'bg-red-900/80 border-red-500/50 text-red-300'}`}
          >
            {toast.msg}
          </motion.div>
        )}
      ...
      <header className="border-b border-white/10 bg-black/80 backdrop-blur sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={fetchAdminData} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg text-sm font-medium transition">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          {STAT_CARDS.map((stat, i) => (
            <div key={i} className={`p-4 rounded-xl border ${stat.color} backdrop-blur-md text-center`}>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={75} paddingAngle={5} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.daily}>
                  ...
                  <Tooltip cursor={{ fill: 'rgba(255,255,255,0.04)' }} contentStyle={{ backgroundColor: '#000', borderColor: '#333', borderRadius: '8px' }} />
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-6">
          <div className="flex flex-wrap gap-4 items-center justify-between mb-6">
            <h3 className="text-lg font-semibold">Food Token Requests</h3>
            <div className="flex gap-3 flex-wrap">
              {(['All', 'Pending', 'Approved', 'Rejected'] as const).map(s => (
                <button key={s} onClick={() => setStatusFilter(s)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold transition ${statusFilter === s ? 'bg-primary text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}>
                  {s}
                </button>
              ))}
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-black/50 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-primary transition-colors"
                />
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filteredRequests.map((req) => (
                  <tr key={req._id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors group">
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${req.food_type === 'Vegetarian' ? 'bg-green-500/15 text-green-400 border border-green-500/30' : 'bg-red-500/15 text-red-400 border border-red-500/30'}`}>
                        {req.food_type}
                      </span>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm text-gray-400">{new Date(req.created_at).toLocaleDateString('en-IN')}</td>
                    <td className="p-4">
                      <div className="flex items-center justify-center gap-2">
                        {/* Quick approve/reject buttons if still Pending */}
                        {req.status === 'Pending' && (
                          <>
                            <button
                              ...
                              title="Approve"
                              className="flex items-center gap-1.5 bg-green-600 hover:bg-green-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              title="View & Reject"
                              className="flex items-center gap-1.5 bg-red-600/30 hover:bg-red-500/50 text-red-400 px-3 py-1.5 rounded-lg text-xs font-bold transition border border-red-500/30"
                            >
      ...
      <AnimatePresence>
        {selectedRequest && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setSelectedRequest(null)}>
            <motion.div
              initial={{ opacity: 0, scale: 0.93, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.93, y: 20 }}
              onClick={e => e.stopPropagation()}
              className="bg-[#0e0e0e] border border-white/10 rounded-2xl p-7 w-full max-w-md shadow-2xl relative"
            >
              <button onClick={() => setSelectedRequest(null)} className="absolute top-4 right-4 text-gray-500 hover:text-white transition">
                <XCircle size={22} />
              </button>
              ...
              <div className="space-y-3 mb-6 text-sm">
                ...
                <div className="flex justify-between items-center border-b border-white/5 pb-3">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${selectedRequest.food_type === 'Vegetarian' ? 'bg-green-500/15 text-green-400' : 'bg-red-500/15 text-red-400'}`}>
                ...
                {selectedRequest.token_id && (
                  <InfoRow label="Token ID" value={selectedRequest.token_id} mono />
                )}
              ...
              {selectedRequest.status === 'Pending' && (
                <div className="flex flex-col gap-3 pt-4 border-t border-white/10">
                  <button
                    ...
                    disabled={actionLoading}
                    className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-green-900/30"
                  >
                    {actionLoading ? <Loader2 className="animate-spin" size={18} /> : <><CheckCircle2 size={18} /> Approve &amp; Generate Token</>}
                  </button>
...
function InfoRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between items-center border-b border-white/5 pb-3">
      ...
      <span className={`text-white ${mono ? 'font-mono text-xs bg-white/10 px-2 py-1 rounded' : 'font-medium'}`}>{value}</span>
    </div>
  );
}
